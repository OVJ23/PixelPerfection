...
# Set the backend to be used by TensorLayerX. Uncomment one of the following lines
# to select a different backend. The default is TensorFlow.
...
# Set the device to be used by TensorLayerX (GPU or CPU)
tlx.set_device('GPU')
...
# Here, we define a set of image transformations that will be applied to both the high-resolution
# and low-resolution training images.
# RandomCrop randomly crops a patch from the image.
# RandomFlipHorizontal flips the image horizontally with a probability of 0.5.
hr_transform = Compose([
    RandomCrop(size=(384, 384)),
    RandomFlipHorizontal(),
])
# Normalize scales the pixel values to the range [-1, 1].
# HWC2CHW converts the image from HWC (height x width x channels) format to CHW (channels x height x width)
# format, which is required by some deep learning frameworks.
...
# Load the high-resolution training images
train_hr_imgs = tlx.vision.load_images(
    path=config.TRAIN.hr_img_path, n_threads=32)

# Define a custom dataset for training, which applies the transformations to the images
# and returns both the low-resolution and high-resolution patches
...
class TrainData(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # Apply the normalization transform to the low-resolution and high-resolution patches
        return nor(lr_patch), nor(hr_patch)

    def __len__(self):
        return len(self.train_hr_imgs)

# Define a module that combines a generator network and a loss function for initialization training
...
# Define a module that combines a discriminator network, a generator network, and a loss function
# for adversarial training of the discriminator
...
class WithLoss_G(Module):
    ...
    def forward(self, lr, hr):
        ...
        g_gan_loss = 1e-3 * \
            self.loss_fn1(logits_fake, tlx.ones_like(logits_fake))
...
def train():
    ...
    net_with_loss_init = WithLoss_init(
        G, loss_fn=tlx.losses.mean_squared_error)
    ...
    for epoch in range(n_epoch):
        ...
        if (epoch != 0) and (epoch % 10 == 0):
            G.save_weights(os.path.join(
                checkpoint_dir, 'g.npz'), format='npz_dict')
            D.save_weights(os.path.join(
                checkpoint_dir, 'd.npz'), format='npz_dict')
...
def evaluate():
    ...
    valid_lr_img = cv2.resize(valid_lr_img, dsize=(
        hr_size1[1] // 4, hr_size1[0] // 4))
    ...
    tlx.vision.save_image(
        valid_lr_img, file_name='valid_lr.png', path=save_dir)
    tlx.vision.save_image(
        valid_hr_img, file_name='valid_hr.png', path=save_dir)
    out_bicu = cv2.resize(valid_lr_img, dsize=[
                          size[1] * 4, size[0] * 4], interpolation=cv2.INTER_CUBIC)
    tlx.vision.save_image(
        out_bicu, file_name='valid_hr_cubic.png', path=save_dir)
